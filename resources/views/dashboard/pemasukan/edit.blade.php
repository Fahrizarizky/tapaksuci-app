...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 col-md-9">
        <h1 class="h3 text-dark text-uppercase fw-bold">Edit Pemasukkan</h1>
    ...
    <div class="card shadow mb-4 col-md-9">
        <div class="card-body">
            <form action="/dashboard/pemasukan/update/{{$pemasukan->id}}" method="POST">
                @csrf
                <div class="form-group">
                    <label for="kode_transaksi" class="font-weight-bold">Kode Transaksi</label>
                    <input type="text" class="form-control" id="kode_transaksi" name="kode_transaksi" value="{{$pemasukan->kode_transaksi}}" required>
                </div>
                <div class="form-group">
                    <label for="tahun_buku" class="font-weight-bold">Tahun Buku</label>
                    <input type="text" class="form-control" id="tahun_buku" name="tahun_buku" value="{{$pemasukan->tahun_buku}}" required>
                </div>
                <div class="form-group">
                    <label for="nama_transaksi" class="font-weight-bold">Nama Transaksi</label>
                    <input type="text" class="form-control" id="nama_transaksi" name="nama_transaksi" value="{{$pemasukan->nama_transaksi}}" required>
                </div>
                <div class="form-group">
                    ...
                    <select type="text" class="form-control" id="kategori" name="kategori" required>
                        ...
                        <option {{$item->nama == $pemasukan->kategori ? 'selected' : ''}}>{{$item->nama}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="nominal" class="font-weight-bold">Nominal</label>
                    <input type="text" class="form-control" id="nominal" name="nominal" value="{{$pemasukan->nominal}}" required>
                </div>
                <div class="form-group">
                    <label for="pengirim" class="font-weight-bold">Diserahkan Oleh</label>
                    <input type="text" class="form-control" id="pengirim" name="pengirim" value="{{$pemasukan->pengirim}}" required>
                </div>
                <div class="form-group">
                    <label for="penerima" class="font-weight-bold">Diterima Oleh</label>
                    <input type="text" class="form-control" id="penerima" name="penerima" value="{{$pemasukan->penerima}}" required>
                </div>
                <div class="form-group">
                    <label for="tanggal_pengiriman" class="font-weight-bold">Tanggal Penyerahan</label>
                    <input type="date" class="form-control" id="tanggal_pengiriman" name="tanggal_pengiriman" value="{{$pemasukan->tanggal_pengiriman}}" required>
                </div>
                <div class="form-group">
                    <label for="lokasi_pengiriman" class="font-weight-bold">Lokasi Penyerahan</label>
                    <input type="text" class="form-control" id="lokasi_pengiriman" name="lokasi_pengiriman" value="{{$pemasukan->lokasi_pengiriman}}" required>
                </div>
                <div class="form-group">
